...
CREATE TABLE USER (
    ...
    primary key(MemberGrade, MemberName)
);
...
CREATE TABLE mainrental (
    ...
    constraint mainrental
    foreign key (MemberGrade)
    references USER (MemberGrade)
);
...
CREATE TABLE Auth (
    ...
    constraint Auth
    foreign key (MemberGrade)useruseruser
    references USER (MemberGrade)
);
